---
description: Install OR upgrade the portable workflow harness in a target project. Copies the portable subset (harness/, commands/, agents/, hooks/) verbatim, generates preference files from the portable templates under harness/templates/preferences/, generates a root CLAUDE.md from the skeleton template, creates docs/ + context/, wires settings.json (guardrail + phase-closing hooks, deny rules), and walks through the one-time credential-helper setup. Idempotent -- re-running upgrades the harness while never overwriting existing preferences or an existing CLAUDE.md.
---
...
- `harness/`   (procedures/, templates/ incl. `claude_md_skeleton.md` and the portable
                `templates/preferences/` files, scripts/ incl. the credential helpers and
                `make_portable_zip.sh`, `USER_MANUAL.md`, `INSTALL.md`, `harness_glossary.md`,
                tests/)
...
Do NOT copy `preferences/`, `approvals/`, `settings.json`, or `context/` here -- those are
...
- `git_parameters.md` ships working defaults (not bare placeholders): the template already
  ...
  `default_branch`, `protected_branch`, `approval_marker_path`), so the hook works before the
  user customizes anything.
...
Do NOT create the approvals directory. It is user-managed exclusively: the guardrail hook and
the settings.json deny rules keep Claude out of that path entirely, and bootstrap is Claude.
The directory comes into being when the USER places the first approval marker with a
`!`-prefixed command at the final-merge gate (that command runs in the user's own shell, not
through Claude's Bash tool). Until then its absence simply means "no marker exists", which the
guardrail hook already treats as "gated merge blocked" -- the safe default.
...
- If it does NOT exist, create it with this content (adjust the hook command's Python
  invocation to the target project's interpreter -- `venv/bin/python` here; a different path
  or bare `python` elsewhere):
  ```json
  {
    "permissions": {
      "deny": [
        ...
        "Read(./.env)",
        "Read(**/.env)",
        "Write(.claude/approvals/**)",
        "Edit(.claude/approvals/**)",
        "Bash(touch .claude/approvals/*)",
        "Bash(cp * .claude/approvals/*)",
        "Bash(mv * .claude/approvals/*)",
        "Bash(tee .claude/approvals/*)"
      ]
    },
    "hooks": {
      "PreToolUse": [
        { "matcher": "Bash",
          "hooks": [ { "type": "command", "command": "venv/bin/python .claude/hooks/git_guardrails.py" } ] }
      ],
      "Stop": [
        { "hooks": [ { "type": "command", "command": "venv/bin/python .claude/hooks/enforce_phase_closing.py" } ] }
      ]
    }
  }
  ```
...
- The `.env` deny rules and the approvals write-deny rules are load-bearing security
  invariants (PAT isolation + user-only approval marker) -- they must end up present.
- Report: created-fresh / merged (list what was added) / already-complete.

### Step 7 -- One-time credential-helper walkthrough

The credential helper (copied verbatim in Step 1 under `harness/scripts/`) lets pushes
authenticate via `GIT_PAT` from the target repo's `.env` without the token ever entering a
transcript. Walk the user through the one-time registration:

1. Confirm `GIT_PAT` is already saved in `TARGET_DIR/.env` (and optionally `GIT_USERNAME`).
   Do NOT read `.env` -- just tell the user this is the precondition.
2. From the target repo root, run the registrar (it configures a repo-local
   `credential.helper` in `.git/config`; it never reads or prints the token):
   ```
   bash <target>/.claude/harness/scripts/setup_credential_helper.sh
   ```
3. Report its `[DONE]`/`[FATAL]` outcome. If the target is not a git repo yet, note that
   `git init` must run first, then re-run the registrar.
...
- Credential helper: registered / pending (git init needed) / skipped.
